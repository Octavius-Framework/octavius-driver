        ...
        when (val response = stream.inputStream.readByte().toInt().toChar()) {
            ...
            'N' -> {
                if (config.mode == SslMode.REQUIRE || config.mode == SslMode.VERIFY_CA || config.mode == SslMode.VERIFY_FULL) {
                    stream.close()
                    throw DriverException(DriverExceptionMessage.SSL_ERROR, "Server does not support SSL, but sslmode=${config.mode.value} was specified.")
                }
            }
            else -> {
                stream.close()
                throw DriverException(DriverExceptionMessage.SSL_ERROR, "Unexpected SSL negotiation response: $response")
            }
        }
    }
}
