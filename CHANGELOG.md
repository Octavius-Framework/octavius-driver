...
- `Throwable.findOctaviusCause()`, which walks a cause chain for the driver's own exception and returns it, or `null` when the failure did not start here. A driver failure that crosses a JDBC boundary rarely arrives as itself - a pool restates it as an `SQLException` of its own, a framework restates that one in turn, and what the caller finally catches names only the layer it came from. It is what the driver now uses wherever it has to decide whether a foreign exception is worth restating, and it replaces the walk the Spring exception translator was carrying separately
- `ownsConnection` on `Connection.getOctaviusSession()`, default `true`. Passing `false` says the connection belongs to something else - Spring's `DataSourceUtils`, a pool borrowed from by hand - and closing the session then undoes the state it left on the connection without giving the connection back, that being its owner's to do
- `OctaviusJdbcTransactionManager`, contributed by the auto-configuration in place of Spring's `JdbcTransactionManager` and carrying the same exception translator and nested-transaction setting. What it adds is an answer for a transaction whose connection has already left
- Column metadata on every result. `row.metadata.getColumn(i)` answers with a `ColumnMetadata`: the name the column came back under, the `PgType` it was read as, the raw `atttypmod` that is the only record of a `numeric(10,2)`'s precision or a `varchar(64)`'s length, and - for a column the server tracked back to a relation - a `ColumnOrigin` naming the relation, its schema, the attribute number and the column's own name, which an alias in the query may have replaced. Nothing is queried for it: every table, view and materialized view already has a row type under the relation's own name, the type load reads them all, and `typrelid` is what ties one to the other. An attribute is found there by its number rather than by its position - a dropped column keeps its number for good, leaving a hole that everything declared after it sits behind, and counting positions would name the wrong column with total confidence. `origin` is `null` for anything that is not a column reference: an expression, a literal, the output of a function. Its **names** are `null` where the type catalog does not describe the relation - composites in `pg_catalog` and `information_schema`, which the type load skips, and any relation created since the last load - so that a stale catalog reads as a relation that could not be named rather than as a value that came from nowhere

#### Changed

- **Breaking:** `IntObjectMap` is internal. It is the open-addressed primitive-key map behind the OID lookups on the read path, and it was public by omission rather than by intent - the two dictionaries that keep one keep it privately, and nothing this API hands out or takes in mentions it
- **Breaking:** `RowMetadata.descriptors` is now `columns`, a list of `ColumnMetadata` rather than of `FieldDescription`, and `FieldDescription` itself is internal. It was the wire form of a `RowDescription` field, where every reference is an OID and every modifier is raw, and it was public only because nothing had yet resolved it into something worth reading. Two of its seven members are gone from the public surface rather than renamed: `formatCode` is the driver's own decision about how it asked for the data, and `dataTypeSize` is a copy of `pg_type.typlen`, a property of the type rather than of the column, carrying nothing that the type does not already say
- A result column whose type the catalog does not describe now fails when the result is described, rather than on the first row that happens to carry a value in it. Such a column could never be decoded anyway - a type the load did not see has no codec either - but it used to depend on the data: a `SELECT *` over `pg_stats` returns rows until one of its `anyarray` columns is non-null, and a `SELECT c FROM pg_class c` succeeds for as long as it matches nothing. The exception now arrives the same way every time, naming the OID, and it is recorded and drained like a failed row mapping rather than thrown out of the middle of an exchange, which would leave the connection a message behind for whoever borrowed it next. One consequence inside: the type dictionary no longer starts empty but seeded from the driver's own builtin codecs, since the query that reads the catalog is itself a result whose columns have to be described, and until it returns those are the only types there are. `TypeDictionary.EMPTY` goes with it: it was the name that state had, nothing outside the registry ever read it, and it existed only to be kept in step with a constructor it could not see
...
- A session closed with a `COPY` still in flight evicts its connection instead of ending the transfer and handing it back. There is no bounded way to end one from the client side: cancelling an export means reading and discarding every row the server has left to produce, and `close()` runs on whoever gave the session back - a `finally`, or a pool reclaiming its connection - so an export abandoned on its first chunk used to hold that thread until the server had finished producing the whole of it. Nothing is lost that the cancel would have kept, a `COPY IN` that never reached `endCopy()` committing nothing either way; the connection goes out with a `warn` line naming the reason, and the pool opens a fresh one in its place
- An index outside a container raises `MappingException(COLUMN_NOT_FOUND)` naming the container and how many positions it has, where the list or array underneath used to raise its own `IndexOutOfBoundsException`. `PgArray.get`, `PgComposite.get`/`set`/`getAttributeOid`, `PgRecord.get`/`getAttributeOid` and `PgMultirange.get` were the accessors that let one through - every other way of asking for a position that is not there already answers with `COLUMN_NOT_FOUND`, `Row.getRaw` and `RowMetadata.getColumn` among them. Inside a converter what changes is the diagnostic: the deserialization context caught the bare exception as a foreign one and restated it as `CONVERSION_ERROR` carrying the JDK's message, where the reason now names what happened. Outside a converter it is the difference between a failure a `catch (e: OctaviusException)` sees and one it does not

#### Fixed

- `IntObjectMap` built with a capacity of one or less came out with a backing table of no slots at all, where the probe mask is `-1` and the first `get` or `put` leaves the array rather than answering. The size is rounded up to a power of two, and one rounds down to zero; the floor is now two slots. Nothing on a live connection sizes one that small - a dictionary is built from a catalog with hundreds of entries in it - so it took a dictionary built from no types at all to reach
...
- `session.transactionState` reported whatever the last ordinary statement had left, for as long as a `COPY` had run since. The status byte was recorded by `QueryExecutor` at the four places it consumes a `ReadyForQuery` itself, and every COPY path reads that message straight off the stream - so a transfer that ended in an error, which is how a cancelled `COPY IN` ends, moved the server into a failed transaction while the session still called itself `IN_TRANSACTION`. It is recorded by `PgStream` now, at the single point that decodes the byte, so no exchange can end without moving it and none has to remember to. The two callers that ask whether a transaction is open before extending `SET SESSION CHARACTERISTICS` to it are more accurate for the same reason
- A session that had already been closed could still reach its connection - by then a pooled one that the next borrower was running queries on. Closing hands the connection back and the pool's proxy goes dead at that moment, but the session holds the physical connection directly as well, and that is how `createNativeQuery`, `createNamedQuery`, `copy`, `largeObjects`, `notifications`, `cancelQuery` and `getSearchPath` reach the wire - none of them passes through the proxy, and the physical connection was open and healthy, so nothing refused them. A session now records that it has been given up, by `close()` or by `abort()`, and everything asked of it afterwards raises `NetworkException(CONNECTION_CLOSED)`. `isValid()` answers `false` rather than raising, the way JDBC has it answer for a closed connection, and a second `close()` returns instead of resetting a connection that belongs to somebody else by then
- Obtaining a session from a pooled `DataSource` reported the pool's refusals in the pool's own terms. `getOctaviusSession()` passed `java.sql.SQLException` straight out - a borrow that timed out as HikariCP's `SQLTransientConnectionException`, a pool already closed as a bare `SQLException`, per-call credentials HikariCP does not implement at all as `SQLFeatureNotSupportedException` - and none of those is a type this API uses anywhere else. They now arrive as `InitializationException(CONNECTION_ERROR)` carrying the original as its cause. Where the pool was restating a failure of the driver's own, that one is raised instead of the restatement wrapped around it: a pool that cannot reach the server reports a timeout, while the exception underneath says the connection was refused, and that is the half worth keeping
- The state-carrying members of a session - `autoCommit`, `readOnly`, `transactionIsolationLevel`, `networkTimeout`, `commit()`, `rollback()` and the savepoint methods - delegate through the JDBC connection on purpose, so that a pool sees the changes it has to reset before handing the connection on, and a proxy answering for a connection it had taken back or evicted raised a bare `java.sql.SQLException` through every one of them. They still delegate; an exception coming back that is not the driver's own is now restated as `NetworkException`, after a look through its cause chain in case the pool was carrying a driver failure it had wrapped on the way
- `OctaviusTemplate` left on the connection everything a session undoes when it closes, because it never closed one: the connection comes from `DataSourceUtils` and goes back the same way, and the session wrapped around it was simply dropped. A `LISTEN` registered inside an `execute` block rode back into the pool with the connection and became the next borrower's starting state, as did a transaction opened by a hand-written `BEGIN`. The session is ended now - outside a transaction by the call that opened it, inside one by a `TransactionSynchronization` running after the commit or rollback and before Spring releases the connection. After rather than before, because a transaction that failed on the server stays failed until the rollback goes through and PostgreSQL ignores every statement sent in the meantime (`25P02`). Resetting any earlier raises on exactly those transactions, and a session that cannot reset its connection gives the connection up instead - so every failed transaction would have cost one, measured as an eviction and a fresh backend on the next borrow. A `COPY` left unfinished still costs the connection rather than being reset away, the same trade a session closed by hand makes
- A transaction works through one session now, rather than one per `execute` call. Every call used to build its own notification, copy and type manager over the connection the transaction already held, and left unsettled which of them was answerable for the state on it. The session is bound to the transaction as a Spring resource, which also unbinds and rebinds it across a suspension - a `REQUIRES_NEW` transaction inside another one runs on a connection of its own, and would otherwise have found the outer transaction's session still bound to the thread
- A connection that left in the middle of a Spring transaction reported the fact as `TransactionSystemException: JDBC commit failed`, and on the rollback path replaced the exception that had caused the rollback - Spring logging `Application exception overridden by rollback exception` over the one diagnostic worth keeping. The driver aborts a connection whose stream is broken or that was left mid-`COPY`, and `session.abort()` does it on request; the pool evicts it and its proxy answers everything afterwards with a bare `SQLException` carrying neither SQLState nor cause, so nothing was left for an exception translator to recognise. The transaction manager now checks the connection before either step - `isClosed()` being the one question an evicted HikariCP proxy still answers rather than raising - and the two paths part company there. A commit raises `OctaviusDataAccessException` over `NetworkException(CONNECTION_ABORTED)`, because the commit did not happen and silence would be a claim about durability that is not true. A rollback stays quiet, because the server discarded the transaction along with the connection and raising there buries the real exception to report a cleanup step that had nothing left to do
