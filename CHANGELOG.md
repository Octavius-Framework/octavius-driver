...
- **`fetchField` treats a missing row and a `NULL` value alike, and the nullability of `T` decides whether either is allowed.** `fetchField<String>()` previously returned `null` when nothing matched, while the same non-nullable `T` raised `MappingException(REQUIRED_ATTRIBUTE_MISSING)` when a row came back carrying `NULL` - the two ways a value can be absent answered differently. Both now raise it, and `fetchField<String?>()` returns `null` for both. This is what the unbounded `T` on the field family is for: `fetchObject<T : Any>` and `fetchRow` have no nullable `T` to read an intention from and still return `null` on an empty result, with `fetchObjectStrict` and `fetchRowStrict` there to demand a row. The `Strict` suffix continues to count rows and nothing else, so `fetchFieldStrict<String?>()` still reports an empty result as `StatementException(INCORRECT_RESULT_SIZE)`. `fetchFields` is unaffected and returns an empty list
- **`fetchField` returns `T` instead of `T?`**, following from the above: a non-nullable `T` can no longer come back null, so declaring the return nullable only forced callers to unwrap a `null` that cannot occur. Nullability now lives in `T` alone and survives the round trip - `fetchField<String>()` hands back a `String`, `fetchField<String?>()` a `String?` - matching how `Row.get<T>()`, `fetchFields<T>(): List<T>` and `fetchFieldStrict<T>(): T` have always read `T`. Existing call sites keep compiling; assignments to a nullable variable still work, and any `!!` or `?:` on the result becomes redundant rather than wrong
- **`PgNotice` is a `data class` of named properties, carrying every field a notice can hold.** A `NoticeResponse` carries the same field set as an `ErrorResponse`, but only `severity`, `code`, `message`, `detail`, `hint` and `where` were exposed, as accessors computed off the raw map - the rest were reachable only by indexing `rawFields` with the protocol's own single-character codes. Measured against a live server, a notice arrives with `file`, `line` and `routine` every time, and `RAISE ... USING` adds `schema`, `table`, `column`, `datatype` and `constraint`. All of them are now real constructor properties built once when the message is read, the same shape `ServerErrorMessage` has, alongside `position`, `internalPosition`, `internalQuery` and a `localizedSeverity` holding the translated severity that `severity` deliberately bypasses. `severity`, `localizedSeverity`, `code` and `message` are non-null, the protocol requiring all four of every notice, so the placeholders that stood in for a missing one - `NOTICE`, `00000`, `Unknown message` - are gone with them. `rawFields` is gone too: the named properties cover every field code the protocol defines, so it held nothing they do not. `equals`, `hashCode` and `copy` come with the data class; the hand-written `toString` is unchanged
- **`ServerErrorMessage.severity` and `code` are non-null, and a `localizedSeverity` joins them.** The protocol requires the severity, the SQLSTATE and the primary message of every `ErrorResponse`, so a null severity or SQLSTATE was never something a server produced - it only forced every caller reading one off an exception to unwrap a value that cannot be absent. `severity` reads the non-localized field and stays safe to compare as a string; the translated severity it deliberately bypasses is now a property of its own, the same pair `PgNotice` carries
...
- **A JDBC URL now overrides the `Properties` it is parsed against, consistently.** `OctaviusProperties.parse(url, info)` applies `info` first and the URL over it, but the host and port were read from the URL's authority *only if `info` had left them unset* - so `getConnection("jdbc:octavius://prod:5432/db", info)` silently connected to whatever host `info` carried. The database name had the opposite fault, being taken from the URL unconditionally and falling back to `postgres`, so a URL naming no database replaced the one `info` supplied. Both now follow one rule: the URL wins where it states something, and where it states nothing it leaves `info` alone. Nothing fills in defaults during parsing any more either - an unstated host, port or database stays `null` until the connection factory resolves it, which is the one place that knows what the defaults are
- A JDBC URL that omits the `/database` part no longer loses its query string. `jdbc:octavius://host:5432?user=consul` swallowed the `?` into the host, so the host became `host:5432?user=consul` and every parameter after it was dropped; the query is now split off before the authority is read
