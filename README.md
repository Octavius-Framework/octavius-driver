...
> 
> The current version is **0.7.0**. The driver is fully capable of handling most database interactions, including complex types (arrays, composites, json) and integrates smoothly with connection pools like HikariCP, but **there is still work to do** before it reaches a 1.0 release. Expect some rough edges.
...
```kotlin
dependencies {
    implementation("io.github.octavius-framework:driver:0.7.0")
}
```
