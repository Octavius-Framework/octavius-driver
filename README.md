...
> 
> The current version is **0.8.1**. The driver is fully capable of handling most database interactions, including complex types (arrays, composites, json) and integrates smoothly with connection pools like HikariCP, but **there is still work to do** before it reaches a 1.0 release.
...
```kotlin
dependencies {
    implementation("io.github.octavius-framework:driver:0.8.1")
}
```
